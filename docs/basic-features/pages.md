...
プリレンダリングすることで結果的にパフォーマンスとSEOが優れます。
...
プリレンダリング時にデータを取得するために、Next.jsでは`getStaticProps`と呼ばれる`async`関数をファイルから`export`します。
この関数はビルド時に呼ばれ、取得されたデータをページの`props`に渡します。
...
つまり、プリレンダリングするページの**パス**は外部のデータに依存します。このような処理を行うために、Next.jsでは`getStaticPaths`という`async`関数を`export`します。（今回の場合、`pages/posts/[id].js`に追加します。）
...
（データ取得の有無に関わらず）可能な限り**静的生成**を行い、ビルドしてCDNによって配信することを推奨しています。
...
例えば、ページの表示に用いるデータ（外部のAPIから取得する）が頻繁に更新される場合を考えましょう。この場合、`getServerSideProps`を追加することで、データを取得して`Page`に渡す処理を行うことができます。
...
`getServerSideProps`は`getStaticProps`とよく似ています。両者の違いは、`getServerSideProps`がビルド時ではなくリクエストの度に呼ばれることです。
...
静的生成では、ページの内容を`export`するか、`getStaticProps`（必要であれば`getStaticProps`も）を`export`してください。
